...
export default function ValComponent({ nodeLabel, nodeId }: ValComponentProps) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) =>
        [...prev, { role: "assistant" as const, content: data.message }].slice(
          -15,
        ),
      );
      ...
      >
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center px-4">
            <p className="text-[#93a0ba] text-sm">
              Ask me anything about {nodeLabel}
            </p>
            <p className="text-[#93a0ba]/60 text-xs mt-1">
              Curious questions unlock new topics
            </p>
          </div>
        ) : (
          ...
          <div className="flex justify-start">
            <div className="bg-[#edf9fe] rounded-2xl rounded-bl-sm px-4 py-3">
              <div className="flex gap-1">
                <span
                  className="w-1.5 h-1.5 bg-[#93a0ba] rounded-full animate-bounce"
